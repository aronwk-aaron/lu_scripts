...
function onStartup(self)
    ...
    -- Load first center piece
	RESMGR:LoadObject { objectTemplate = gCenterLOT, x=-4.0, y=200.00, z= 103.0, owner = self } 
end
...
function onChildLoaded(self, msg)
	...
	if msg.templateID == gCenterLOT then
		-- childID
		if gCenterSpawned == 0 then
			gCenterSpawned = 1
			gCenterIDs.center1 = msg.childID
			RESMGR:LoadObject { objectTemplate = gCenterLOT , x=-12.0, y=200.00, z=-111.0 , owner = self } 
		
		elseif gCenterSpawned == 1 then
			gCenterSpawned = 2
			gCenterIDs.center2 = msg.childID
		end
	end 
end
