...
function onStartup(self,msg)
    ...
    -- Process LOT ID for built state
    local myLot = self:GetLOT().objtemplate
    local built = true
    if((myLot == 14202) or (myLot == 14206) or (myLot == 14207) or (myLot == 14208) or (myLot == 14209) or (myLot == 14210) or (myLot == 14211) or (myLot == 14213)) then
        -- We are a quickbuild wall
        built = false
    ...
    self:SetVar("built", built)
    
end

function onRebuildComplete(self, msg)
    -- We are a QB that has just been built
...
function onDie(self, msg)
    ...
    local team = self:GetVar("team") or 0
    local config = { {"rebuild_activators", posString }, {"respawn", 100000 }, {"rebuild_reset_time", -1}, {"no_timed_spawn", true}, {"currentTime", 0} , {"CheckPrecondition" , "21"}, {"team", team}, {"custom_script_client", "scripts/TestAndExample/test_aaronB/L_CASTLE_DEFENSE_WALL_CLIENT.lua"}, {"custom_script_server", "scripts/TestAndExample/test_aaronB/L_CASTLE_DEFENSE_WALL_SERVER.lua"} }
