...
function onScriptNetworkVarUpdate(self, msg)
	...
	for varName,varValue in pairs(msg.tableOfVars) do
		...
		elseif varName == "PlayAudio" and varValue then
			self:PlayNDAudioEmitter{m_NDAudioEventGUID = varValue}	
		elseif varName == "StopAudio" and varValue then
			self:StopNDAudioEmitter{m_NDAudioEventGUID = varValue}
		elseif varName == "PlayAudio2" and varValue then
			self:PlayNDAudioEmitter{m_NDAudioEventGUID = varValue}	
		elseif varName == "StopAudio2" and varValue then
			self:StopNDAudioEmitter{m_NDAudioEventGUID = varValue}
		end
	end
end
