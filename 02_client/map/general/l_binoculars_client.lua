...
function onRenderComponentReady(self,msg) 
	-- Get the player
	local player = GAMEOBJ:GetLocalChar()
...
function onTimerDone (self,msg)
    -- Obtain a reference to the player and the controlled object (which may be the same or may be a different object)
    local player = GAMEOBJ:GetLocalChar()
	local controlled = GAMEOBJ:GetControlledID()
	    
    if (msg.name == "startBinoc" ) then	
		local oDir = self:GetRotation()
		
		GAMEOBJ:GetTimer():AddTimerWithCancel( 0.5, "lookanimation",self )
		controlled:SetStunned{StateChangeType = "PUSH", bCantMove = true, bIgnoreImmunity = true, bCantTurn = true, bCantEquip = true}
		controlled:SetRotation{ x = oDir.x , y = oDir.y, z =oDir.z , w=oDir.w}
		-- Temp animation while we wait for an animation of the player looking through the binoculars	
		controlled:PlayAnimation{animationID = "binoculars-idle"} 
	...
	elseif (msg.name == "lookanimation") then
	    ...
	    if(cName) then
	        -- Play cinematic
            player:PlayCinematic {pathName = cName, bIsSkippable = true} 
		...
		controlled:SetStunned{StateChangeType = "POP", bCantMove = true, bIgnoreImmunity = true, bCantTurn = true, bCantEquip = true}
...
----------------------------------------------
function notifyOnHit(self, controlled, msg)
...
function binocularExit(self)
    -- Obtain a reference to the player (or their controlled object)
    local controlled = GAMEOBJ:GetControlledID()
    
    -- Remove cinematic bars
	UI:SendMessage("popGameState", {{"state", "cinematic"}})
	-- Exit interaction
	controlled:TerminateInteraction{type = 'fromInteraction', ObjIDTerminator = self}
    -- Reset animation
	controlled:PlayAnimation{animationID = "ben_is_king"} 
	self:SetVar("beingUsed", false)
	
	-- Cancel the notification requests
	self:SendLuaNotificationCancel{requestTarget = controlled, messageName = "OnHit"}
