...
function onRebuildNotifyState(self, msg)
	...
	if (msg.iState == CONSTANTS["RebuildStateCompleted"]) then
        ...
        for i, object in pairs(friends) do
            if (object:GetLOT().objtemplate == 6537) or (object:GetLOT().objtemplate == 6539) then
                object:NotifyObject{name = "killQBTurret2", ObjIDSender = self}
            end
        end
    end
end
...
print("turret down!")
    ...
    for i, object in pairs(friends) do
        if (object:GetLOT().objtemplate == 6537) or (object:GetLOT().objtemplate == 6539) then
            object:NotifyObject{name = "dontShootQBTurret2", ObjIDSender = self}
        end
    end
end