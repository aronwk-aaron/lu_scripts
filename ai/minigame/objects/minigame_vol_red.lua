...
function onCollisionPhantom (self,msg)
	...
	if ( player:GetFaction().faction == 100 ) then 
	
        local item = player:GetEquippedItemInfo{ slot = "hair" }.lotID 
          -- Check to see if my flag is here
          
        if  item == GAMEOBJ:GetZoneControlID():GetVar("Set.BlueFlag_LootOBJ") and GAMEOBJ:GetZoneControlID():GetVar("Con.Red_Flag_Home") then
            for i =0,  player:GetInventorySize{inventoryType = 1 }.size do
                if player:GetInventoryItemInSlot{slot = i }.itemID:Exists() then
                    ...
                    if player:GetInventoryItemInSlot{slot = i }.itemID:GetLOT{}.objtemplate == GAMEOBJ:GetZoneControlID():GetVar("Set.BlueFlag_LootOBJ") then
                        ...
                        GAMEOBJ:GetZoneControlID():NotifyClientZoneObject{name = "sendToAllclients_bubble" , paramStr = player:GetName().name.." Captured The Blue Flag!!" }
...
function onTimerDone(self, msg)
    if msg.name == "CaptedFlag" then
         ...
         if  team_2 == GAMEOBJ:GetZoneControlID():GetVar("Set.Score_To_Complete") then
                  ...
                  GAMEOBJ:GetZoneControlID():NotifyClientZoneObject{name = "sendToAllclients_bubble" , paramStr =  "Red Team Won the Match!!" }
     ...
     if msg.name == "CaptedFlag2" then
          ...
          GAMEOBJ:GetZoneControlID():NotifyClientZoneObject{name = "sendToAllclients_bubble" , paramStr = "Reseting Blue Flag in 5 Seconds!" }
    ...
    if msg.name == "CaptedFlag3" then
          ...
          local redFlag =  getObjectByName( GAMEOBJ:GetZoneControlID(), "Blue_Flag_1")
          local redpos =   redFlag:GetPosition().pos 
          RESMGR:LoadObject { objectTemplate =  GAMEOBJ:GetZoneControlID():GetVar("Set.BlueFlagOBJ") , x= redpos.x, y= redpos.y , z= redpos.z, owner =  GAMEOBJ:GetZoneControlID() }
    end
