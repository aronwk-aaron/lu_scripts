...
self:SetVar("TurretLifeTime", 30)
setState("AiDisable",self)
GAMEOBJ:GetTimer():AddTimerWithCancel( self:GetVar("TurretLifeTime")  , "TurretLifeTime", self )
----------------------------------*
end
...
onTimerDone = function(self, msg)
    if msg.name == "TurretLifeTime" then
    
            local player = getObjectByName(self, "playerID")
            if player ~= nil then
                self:RebuildCancel{ userID  = player }
            end
             setState("AiDisable",self)
