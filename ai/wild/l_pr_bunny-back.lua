...
function onStartup(self)
    ...
    local friends = self:GetObjectsInGroup{ group = "Panto" }.objects

    for i = 1, table.maxn (friends) do 
        if friends[i]:GetLOT().objtemplate == 6558 then
            self:FollowTarget { targetID = friends[i], radius = 5, speed = 0.75, keepFollowing = true }
        end
    end
