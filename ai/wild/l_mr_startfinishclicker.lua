...
function onUse(self, msg)
    ...
    for i = 1, table.maxn (friends) do 
        if friends[i]:GetLOT().objtemplate ~= 6284 then
...
function onNotifyObject(self, msg)
    ...
    if msg.name == "trapplaced" then
        if #spawns >= LoadIndex and LoadIndex > PlaceIndex then
            sections[#sections]:SetPosition{pos={x = msg.ObjIDSender:GetPosition{}.pos.x, y = msg.ObjIDSender:GetPosition{}.pos.y, z = msg.ObjIDSender:GetPosition{}.pos.z}}

            PlaceIndex = PlaceIndex + 1
            sections[#sections]:NotifyObject{name = "Change", ObjIDSender = self}
    ...
    elseif msg.name == "traplabel" then
