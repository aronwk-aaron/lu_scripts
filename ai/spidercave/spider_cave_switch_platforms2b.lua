...
function onFireEvent(self, msg)
    ...
    local object = self:GetObjectsInGroup{group = "SpiderPsSwitch1", ignoreSpawners = true}.objects[1]
    if object then
   ...
   elseif msg.args == "up" then
      self:SetVar("IAMengaged", false)
      local object2 = self:GetObjectsInGroup{group = "SpiderPsSwitch1", ignoreSpawners = true}.objects[1]
      if object2 then
