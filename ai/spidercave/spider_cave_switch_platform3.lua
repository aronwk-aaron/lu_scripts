...
function onFireEvent(self, msg)
    ...
    self:SetVar("IAMengaged", true)
      local object = self:GetObjectsInGroup{group = "SpiderP2Switch2", ignoreSpawners = true}.objects[1]
      if object then
   ...
   elseif msg.args == "up" then
      self:SetVar("IAMengaged", false)
      local object2 = self:GetObjectsInGroup{group = "SpiderP2Switch2", ignoreSpawners = true}.objects[1]
      if object2 then
