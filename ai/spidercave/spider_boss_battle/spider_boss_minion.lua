...
--------------------------------------------------------------------------------
	...
	local boss = getObjectByName(ActivityObj, "bossObj")
	local Markpos = boss:GetPosition().pos 

	self:GoTo { speed = 20, target = { x =Markpos.x ,z =Markpos.z ,y =Markpos.y } }
...
function onTemplateDie(self,msg)
	...
	local ActivityObj = self:GetParentObj().objIDParent
	ActivityObj:NotifyObject{name = "removeMinion" ,ObjIDSender = self }
