...
function WayPointEventFunc(self,name,value)
	...
	if name == "spawnSpiders" then
	
	 	local ActivityObj = self:GetObjectsInGroup{ group = "ActivityObj" ,ignoreSpawners = true }.objects
		ActivityObj[1]:NotifyObject{name = "spawnMinions" }	
	
	
	end
	if name == "spawnSpiders2" then
	
	 	local ActivityObj = self:GetObjectsInGroup{ group = "ActivityObj" ,ignoreSpawners = true }.objects
		ActivityObj[1]:NotifyObject{name = "spawnMinions2" }	
    ...
    if name == "bounce" then
       ...
       local  tpos =  LEVEL:GetPathWaypoints(self:GetVar("attached_path"))[(self:GetVar("WPEvent_NUM") + 1)].pos
       self:BouncePlayer{ niDestPt = tpos , fSpeed = value , disableMovement = true, bStickLanding = true , ObjIDBouncer =  self }
        -- Update Boss GUI
     
       SendNetWorkVar( self , "intGUI"  , "", "", "", "", "", "" )
