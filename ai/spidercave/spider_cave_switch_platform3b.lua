...
function onFireEvent(self, msg)
    ...
    self:SetVar("IAMengaged", true)
      local object = self:GetObjectsInGroup{group = "SpiderP2Switch1", ignoreSpawners = true}.objects[1]
      if object then
         object:NotifyObject{name = "pressed", ObjIDSender = self}
      end
      local object = self:GetObjectsInGroup{group = "SpiderPlatformFinal", ignoreSpawners = true}.objects[1]
      if object then
         object:GoToWaypoint{iPathIndex = 1, bAllowPathingDirectionChange = true}
      end
   elseif msg.args == "up" then
      self:SetVar("IAMengaged", false)
      local object2 = self:GetObjectsInGroup{group = "SpiderP2Switch1", ignoreSpawners = true}.objects[1]
      if object2 then
