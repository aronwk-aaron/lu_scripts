...
function onFireEvent(self, msg)
   if msg.args == "down" and self:GetVar("ModuleUp") == true then
    self:SetVar("IAMengaged", true)
      local object = self:GetObjectsInGroup{group = "SpiderPSwitch2", ignoreSpawners = true}.objects[1]
      if object then
         object:NotifyObject{name = "pressed", ObjIDSender = self}
      end
      local object = self:GetObjectsInGroup{group = "SpiderPlatform", ignoreSpawners = true}.objects[1]
      if object then
         object:GoToWaypoint{iPathIndex = 1, bAllowPathingDirectionChange = true}
      end
   elseif msg.args == "up" then
      self:SetVar("IAMengaged", false)
      local object2 = self:GetObjectsInGroup{group = "SpiderPSwitch2", ignoreSpawners = true}.objects[1]
      if object2 then
