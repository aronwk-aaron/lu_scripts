...
--]]
    ...
    Set['wanderRadius']     = 12      -- Wander Radius
    --- FOV Radius --
    -- Aggro
...
function onFireEvent( self, msg )
	if ( msg.args ) then
		if self:GetVar("ambushFoo") == nil then
--~ 			print("*****triggered fireEvent path movement **************")
			if self:GetVar("teleLoc") then
				local s = self:GetVar("teleLoc")
				local t = split(s, '_')
				local xSplit= {x=t[1] , y=t[2] , z=t[3] }
--~ 				for k,v in pairs(xSplit) do print(x) end

				self:Teleport{ pos = xSplit}
			end

--~ 			WayPointEventFunc(self,"changeWP","Ambush01,3")
			WayPointEventFunc(self,"changeWP",self:GetVar("pathVar")..",1")
			self:SetVar("ambushFoo", "asdf")
	    end
	end
...
function onTemplateCustomStateEnter(self, msg)



end

function onTemplateCustomStateArrived(self, msg)
