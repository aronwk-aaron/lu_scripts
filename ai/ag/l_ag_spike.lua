...
--///////////////////////////////////////////////////////////////////////////////////////
--//           Robotonist Spawer Script
--///////////////////////////////////////////////////////////////////////////////////////

function onStartup(self)
    
     storeObjectByName(self, "crystal", self)
    ...
    --/////////////////////////////////////////////
    self:SetVar("PetNames", "robotonist") 
    self:SetVar("WP_Alpha", "bot")   -- starting letter of the Way Point ( a_1 = a)  or (anyName_1) 
    self:SetVar("MaxPets", 3)      -- Max Pets Spawn in the wrold
    self:SetVar("TotalPets", 10)    -- Total Number of Pets spawn points
    self:SetVar("PetID", 4019) 
    --/////////////////////////////////////////////
    --/////////////////////////////////////////////
...
function SpawnPet(self, num ) 
     for i = 1,  self:GetVar("TotalPets")  do  
        if num == i then
             ...
             local config = { {"pathset", self:GetVar("WP_Alpha").."_"..i} , {"renderDisabled", false} }
             
             RESMGR:LoadObject { objectTemplate =  self:GetVar( "Pet_"..i )  , x=  firstWP.x  , y=  firstWP.y , z=  firstWP.z  , owner = self ,configData = config } -- A1  
...
function onNotifyObject(self, msg)
    ...
    -- set the state
    if (msg.name == "DeadChild") then
		--print("Parent Enter DeadChild State****************************************") 
		local NChildern =  self:GetVar("NumberOfChildern") - 1 
		self:SetVar("NumberOfChildern", NChildern ) 
		setState("ReSpawnChild",self)
	end
