...
function freezePlayer(self, bFreeze)    
    local controlledObj = GAMEOBJ:GetControlledID()
    local eChangeType = "POP"
    
    if bFreeze then
        if controlledObj:IsDead().bDead then
            --print('frozen')
    ...
    local stunSucceeded = controlledObj:SetStunned{ StateChangeType = eChangeType,
                        bCantMove = true, bCantAttack = true, bCantUseItem = true, bCantInteract = true }.bCantMoveOutChangeWasApplied
                        
    --print('Player ' .. controlledObj:GetName().name .. ' ' .. eChangeType .. ' is frozen: ' .. tostring(self:GetVar('frozen')) .. ' ' .. tostring(playerID:GetStunned().bCanMove))
    if (stunSucceeded == false) and (eChangeType == "PUSH") then
        print(controlledObj:GetName().name .. ' is still able to move')
        GAMEOBJ:GetTimer():AddTimerWithCancel( 0.1, "Try_Freeze_Again", self )
    end
end
