...
function onCollisionPhantom(self, msg)    
    ...
    if not self:GetVar('isTriggered') then
        ...
        if parentObj then -- fire the correct event to this objects parent
			-- use local player's ID instead of controlled, since controlled can change
			local playerID = GAMEOBJ:GetLocalCharID()
            if nodeNum == 1 then      
                parentObj:FireEvent{senderID = self, args = 'PlayerHitFirstGoal_' .. playerID}                    
            elseif nodeNum == nodeTotal then
                parentObj:FireEvent{senderID = self, args = 'PlayerWon_' .. playerID}                
            else
                parentObj:FireEvent{senderID = self, args = 'PlayerHitGoal_' .. playerID}
            end
        else
