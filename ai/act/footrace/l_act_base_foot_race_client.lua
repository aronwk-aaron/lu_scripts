...
require('client/ai/AG/L_AG_NPC_NO_INTERACT')
...
function onCheckUseRequirements(self, msg)
	...
	-- allow these to be started while on a mount
	msg.bAllowsWhileOnMounts = true
...
function onNotifyClientObject(self, msg)
    ...
    elseif msg.name == "stop_timer" then              
        ...
        StopFootRace(self, GAMEOBJ:GetLocalChar())    
    end
end
...
function baseFireEvent(self, msg)
    ...
    if tArgs[2] then
        ...
        if tArgs[2] ~= GAMEOBJ:GetLocalCharID() then 
            return
        else
...
function baseActivityTimerDone(self, msg)
    ...
    elseif msg.name == "Foot_Race_Timer" then
        ...
        ----------------------------------------------------------------------------------------------------------------------
        UI:SendMessage("UpdateFootRaceScoreboard",  {{"visible", false }} )        
        GAMEOBJ:GetLocalChar():DisplayTooltip{ bShow = true, strText = Localize("FOOT_RACE_FAIL"), iTime = 1 }
        StopFootRace(self)
