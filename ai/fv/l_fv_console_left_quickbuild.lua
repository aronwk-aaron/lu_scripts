---------------------------------------------------------------
--left console to be rebuilt to cause the blue brick to become available
---------------------------------------------------------------
...
function onRebuildNotifyState(self, msg)
   if msg.iState == 2 then
      ...
      if object then
         ...
         object:NotifyObject{name = "ConsoleLeftUp", ObjIDSender = self}
      end
   ...
   elseif msg.iState == 4 then
      ...
      if object then
         ...
         --object:PlayFXEffect{name = "imaginationbase", effectID = 114, effectType = "onrebuild"}
         object:NotifyObject{name = "ConsoleLeftDown", ObjIDSender = self}
      end
   end
end
...
function onUse(self, msg)
	...
	if self:GetVar("IAmBuilt") == true then
		...
		if object then
			...
			object:NotifyObject{name = "ConsoleLeftActive", ObjIDSender = self}
		end
	end
