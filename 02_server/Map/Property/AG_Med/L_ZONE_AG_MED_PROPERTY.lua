...
local Spawners = {
					Enemy 			= { "StrombieWander","Strombies","Mechs","OtherEnemy" }, -- this can be as many spawner networks as necessary, 
					...
					BehaviorObjs	= {}						-- any other spawner networks that need to be activated and destroyed for the maelstrom pushback
				}
-- player flags. These have to be different for each property map. these are set up in the db
local flags = { 
				defeatedProperty 	= 118,			-- when the player builds the claim marker, this flag is set
				placedModel 		= 119,			-- when a player places a model for the first time, this flag is set
				guardMission		= 1293,			-- last mission for the guard
				password			= "s3kratK1ttN",-- behavior password build qb object with behaviors
				generatorID			= 10118,		--lot id of the generator
				orbID				= 10226,		-- lot id of the orb
				behavQBID			= 10445,		-- lot id of the behavior platform quickbuild
				brickLinkMissionID  = 1294			-- Achievement ID to complete on property rental
			  }
