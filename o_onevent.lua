...
function onOnHit(self,msg)
    ...
    if self:IsEnemy{ targetID = msg.attacker }.enemy == true and not msg.attacker:IsDead().bDead and not self:IsDead{}.bDead and self:GetVar("FleeStatus") ~= 1  and not self:GetVar("AiDisabled") then
...
function onNotifyObject(self, msg)
    if (onTemplateNotifyObject and  onTemplateNotifyObject(self,msg) == true) then
           return
    end 
	if msg.name == "GoHome" then
	 	setState("GoHome",self)
