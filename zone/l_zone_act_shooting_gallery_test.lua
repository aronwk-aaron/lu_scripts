...
--------------------------------------------------------------
AddWave(waves,	5.0,	"Wave;One"   )		
AddWave(waves,	5.0,	"Wave;Two" )
AddWave(waves,	5.0,	"Wave;Three" )
--------------------------------------------------------------
-- Startup
--------------------------------------------------------------
...
--------------------------------------------------------------
	...
	AddSpawn(spawn,paths,
		 2974,	0.0,	2.0,	true,	0,	2,	2.0,	100,		false,	0.0,	1.0,	1.0,	false,		false,		0.0)
	...
	AddSpawn(spawn,paths,
		 2983,	0.0,	2.0,	true,	0,	2,	10.0,	100,		false,	0.0,	1.0,	1.0,	true,		true,		0.0)
	...
	AddSpawn(spawn,paths,
		 5946,	5.0,	10.0,	true,	5.0,	10.0,	4.0,	250,		false,	0.0,	1.0,	1.0,	false,		true,		0.0)
	...
	AddSpawn(spawn,paths,
		 2565,	10.0,	15.0,	true,	10.0,	15.0,	2.0,	500,		false,	0.0,	1.0,	1.0,	true,		true,		0.0)
	...
	--------------------------------------------------------------
	--AddPath(paths,	"Pearl")
    --AddPath(paths,	"Pearl_2")
    --AddPath(paths,	"Pearl_3")
	--AddSpawn(spawn,paths,
	--	 2976,	5.0,	10.0,	true,	5.0,	10.0,	2.0,	500,		false,	0.0,	1.0,	1.0,	true,		true,		0.0)
...
--------------------------------------------------------------
    ...
    AddPath(paths,	"Wave_1_Ship_2")
	AddSpawn(spawn,paths,
		 2974,	0.0,	2.0,	true,	0,	2,	2.0,	200,		false,	0.0,	1.0,	1.0,	false,		false,		0.0)
	...
	AddPath(paths,	"Wave_1_Sub_2")
	AddSpawn(spawn,paths,
		 2983,	0.0,	2.0,	true,	0,	2,	2.0,	200,		false,	0.0,	1.0,	1.0,	true,		true,		0.0)
	...
	AddPath(paths,	"Wave_1_Duck_1")
	AddPath(paths,	"Wave_1_Duck_2")
    AddPath(paths,	"Wave_2_Duck_1")
	AddSpawn(spawn,paths,
		 5946,	5.0,	10.0,	true,	5.0,	10.0,	4.0,	500,		false,	0.0,	1.0,	1.0,	false,		false,		0.0)
	...
	AddSpawn(spawn,paths,
		 2565,	10.0,	15.0,	true,	10.0,	15.0,	2.0,	1000,		false,	0.0,	1.0,	1.0,	true,		true,		0.0)
...
--------------------------------------------------------------	
    ...
    AddPath(paths,	"Wave_1_Ship_2")
	AddSpawn(spawn,paths,
		 2974,	0.0,	2.0,	true,	0,	2,	2.0,	300,		false,	0.0,	1.0,	1.0,	false,		false,		0.0)
	...
	AddPath(paths,	"Wave_1_Sub_2")
	AddSpawn(spawn,paths,
		 2983,	0.0,	2.0,	true,	0,	2,	2.0,	300,		false,	0.0,	1.0,	1.0,	true,		true,		0.0)
    ...
    AddPath(paths,	"Wave_2_Duck_1")
	AddSpawn(spawn,paths,
		 5946,	5.0,	10.0,	true,	5.0,	10.0,	4.0,	750,		false,	0.0,	1.0,	1.0,	false,		false,		0.0)
    ...
    AddPath(paths,	"Wave_2_Ness_1")
	AddSpawn(spawn,paths,
		 2565,	10.0,	15.0,	true,	10.0,	15.0,	2.0,	1500,		false,	0.0,	1.0,	1.0,	true,		true,		0.0)
...
function startGame(self)
	...
	if ((cannon) and (player)) then
		...
		if ((cannon:GetActivityUser().userID):GetID() ~= player:GetID()) then
			...
			print("starting game")
			getObjectByName( "Cannon_Timer"):NotifyObject{ name = "Start" }
...
function stopGame(self, bCanceling)
	...
	if ((cannon) and (player)) then
		...
		print("stopping game")
        getObjectByName( "Cannon_Timer"):NotifyObject{ name = "Stop" }
...
function DoGameStartup(self)
	...
	LOCALS["ThisWave"] = 1
	if CONSTANTS["FIRST_WAVE_START_TIME"] < 1 then
	     CONSTANTS["FIRST_WAVE_START_TIME"] = 1
	end
...
function SpawnObject(num, spawn, self, bSpawnNow)
	...
	if (bSpawnNow == true) then
		if (spawn.initSpawnTimeMin > 0 and spawn.initSpawnTimeMax > 0) then
			...
			GAMEOBJ:GetTimer():AddTimerWithCancel( 1, timerName, self )
		end
    ...
    if (string.starts(msg.name,"SpawnWave")) then
		...
		if (LOCALS["GameStarted"] == true) then
			...
			if (tonumber(waveNum) >= CONSTANTS["NUM_WAVES"]) then
                print("****************************** asdfaaaaaaaa")
				-- setup next wave
				if  waves[tonumber(waveNum)].timeLimit < 1 then
				     myTimer = 1
				else
				      myTimer = waves[tonumber(waveNum)].timeLimit
				end
				
				GAMEOBJ:GetTimer():AddTimerWithCancel( 0.1 , "GameOver", self )

			else
               local  myTimer = waves[tonumber(waveNum)].timeLimit
				-- setup next wave
				GAMEOBJ:GetTimer():AddTimerWithCancel( myTimer, "EndWave" .. waveNum,self )
    ...
    if (string.starts(msg.name,"EndWave")) then
		...
		if (LOCALS["GameStarted"] == true) then
			...
			if (waveNum > CONSTANTS["NUM_WAVES"]) then
				...
				-- setup next wave
				GAMEOBJ:GetTimer():AddTimerWithCancel( 1, "GameOver", self )
...
function onNotifyObject(self, msg)
     if msg.name == "Cannon_Timer" then
        storeObjectByName( "Cannon_Timer", msg.ObjIDSender )
     end
