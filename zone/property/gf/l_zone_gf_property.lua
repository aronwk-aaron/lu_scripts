...
local Group = {
				ClaimMarker		= "BehavQB", -- claimmarker object that the player rebuilds
...
local Spawners = {
					Enemy 			= { "PiratesWander","PiratesGen","AdmiralsWander","AdmiralsGen" }, -- this can be as many spawner networks as necessary, 
												--but all spawner networks with enemies should be listed
					ClaimMarker 	= "BehavPlat", --the spawner network for the claim marker, should only be one node
					...
					AmbientFX		= { "Birds","Falls"},  -- the ambient happy effects for the property, they are on by default and are turned off if maelstrom is spawned
					BehaviorObjs	= { "TrappedPlatform", "IceBarrier","FireBeast" } -- Behavior scenerio object
				}
-- player flags. These have to be different for each property map. these are set up in the db
local flags = { 
				defeatedProperty 	= 98,           -- when the player builds the claim marker, this flag is set
				placedModel 		= 106,	        -- when a player places a model for the first time, this flag is set
				guardMission		= 873,          -- last mission for the guard
				password			= "s3kratK1ttN",-- behavior password build qb object with behaviors
				generatorID			= 11109,        -- lot id of the generator
				orbID				= 10226,        -- lot id of the orb
				behavQBID			= 11001,        -- lot id of the behavior platform quickbuild
