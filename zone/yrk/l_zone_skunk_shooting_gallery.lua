...
CONSTANTS["PLAYER_START_POS"] = {x = -905.00, y = 234, z = 2.31}
CONSTANTS["PLAYER_START_ROT"] = {w = 0.91913521289825, x = 0, y = 0.39394217729568, z = 0}
CONSTANTS["SPAWN_PATH_NAME"] = "skunknodes"
SPAWNPOINTS = {}

-- cannon constants
CONSTANTS["CANNON_TEMPLATEID"] = 3879
CONSTANTS["CANNON_PLAYER_OFFSET"] = {x = -5.624, y = 0, z = -2.451}
CONSTANTS["CANNON_VELOCITY"] = 80.0
CONSTANTS["CANNON_MIN_DISTANCE"] = 10.0
...
AddWave(waves,	45.0,	"Wave;Two"   )
AddWave(waves,	45.0,	"Wave;Three" )
AddWave(waves,	60.0,	"Wave;Four" )


-- wave constants
CONSTANTS["NUM_WAVES"] = #waves
CONSTANTS["FIRST_WAVE_START_TIME"] = 4.0
CONSTANTS["IN_BETWEEN_WAVE_PAUSE"] = 3.0

CONSTANTS["STREAK_MOD"] = 2
CONSTANTS["STREAK_BONUS"] = {1,2,5,10}
...
--------------------------------------------------------------
	...
	paths = {}
	--------------------------------------------------------------
	AddPath(paths,	"Skunk1")
	AddPath(paths,	"Skunk2")
	AddSpawn(spawn,paths,
		 3719,	4,	10,	true,	4,	10,	4.0,	50,		false,	0.0,	1.0,	1.0,	false,		false,		0.0)
	...
	paths = {}
	--------------------------------------------------------------
	AddPath(paths,	"BabySkunk1")
	AddPath(paths,	"BabySkunk2")
	AddPath(paths,	"BabySkunk3")
	AddPath(paths,	"BabySkunk4")
	AddPath(paths,	"BabySkunk5")
	AddPath(paths,	"BabySkunk6")
	AddPath(paths,	"BabySkunk7")
	AddSpawn(spawn,paths,
		 3899,	0.1,	0.1,	true,	0.1,	0.1,	4.0,	25,		false,	0.0,	1.0,	1.0,	false,		false,		0.0)
...
--------------------------------------------------------------
	...
	paths = {}
	--------------------------------------------------------------
	AddPath(paths,	"MamaSkunk1")
	AddPath(paths,	"MamaSkunk2")
	AddPath(paths,	"MamaSkunk3")
	AddSpawn(spawn,paths,
		 3898,	5,	15,	true, 5,	15,	4.0,	100,		false,	0.0,	1.0,	1.0,	false,		false,		0.0)
...
function setCannonReticuleSize(self, waveNum)
	local cannon = getObjectByName("cannonObject")
	cannon:SetShootingGalleryReticuleEffect{ inRangeType = "SkunkSG" }
end
...
function LoadSpawnPoints(self)

	local pathMsg = LEVEL:GetPathWaypoints(CONSTANTS["SPAWN_PATH_NAME"])
...
onTimerDone = function(self, msg)
    ...
    if (string.starts(msg.name,"EndWave")) then
		...
		if (LOCALS["GameStarted"] == true) then
			...
			-- play wave animation on actors, cannon, and player
			PlaySceneAnimation("skunkwave" .. LOCALS["ThisWave"], true, true)
    ...
    if (string.starts(msg.name,"DoSpawn")) then
		...
		if (LOCALS["GameStarted"] == true) then
			...
			local config = {{"spawndata", SpawnData},
							{"streakmod", CONSTANTS["STREAK_MOD"]},
							{"streakbonus", CONSTANTS["STREAK_BONUS"]}}
...
function onChildLoaded(self, msg)
	...
	local SpawnData = msg.childID:GetVar("spawndata")
...
function onNotifyObject(self, msg)
	if msg.name == "escaped" then
			--skunk has escaped, penalize player
			local score = msg.ObjIDSender:GetActivityPoints()
			score.points = score.points * -1
			msg.ObjIDSender:NotifyObject{name = "setpoints", param1 = score.points, param2 = score.addTime}
			getObjectByName("cannonObject"):UpdateMissionTask{target = msg.ObjIDSender, taskType = "kill"}
	end
end