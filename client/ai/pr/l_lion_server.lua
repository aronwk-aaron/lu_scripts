--------------------------------------------------------------
-- Server side script on the lion pet
-- this script control the lions life before he is tamed by a player
...
function onStartup(self)
	...
	self:SetNetworkVar("liontamer", player)
...
function onNotifyPetTamingMinigame(self,msg)
	...
	elseif  msg.notifyType == "SUCCESS" then	
		...
		print("remove object from group lion"..player:GetID())
		self:RemoveObjectFromGroup{group = "lions"}
		self:RemoveObjectFromGroup{group = "lion"..player:GetID()}
		
		local lions2 = self:GetObjectsInGroup{ group = "lions", ignoreSpawners = true }.objects
		print("after "..#lions2)
	end
