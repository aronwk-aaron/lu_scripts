...
function onClientUse(self, msg)
    ...
    -- do anims on player
	local anims = self:GetVar("interactionPlayerAnim")
	if anims and #anims > 0 then
		-- get a random animation
		local num = math.random(1, #anims)
		local controlled = GAMEOBJ:GetControlledID()
		controlled:PlayAnimation{animationID = anims[num]}
	end
...
function onProximityUpdate(self, msg)
    ...
    if ( (controlled) and (controlled:Exists()) ) then
		if msg.status == "ENTER" and msg.objId:GetID() == controlled:GetID() then
