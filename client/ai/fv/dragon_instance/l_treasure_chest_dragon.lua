...
--
-- updated by mrb... 1/24/11 - turn off physics when we hide the object
--------------------------------------------------------------
...
function onCheckUseRequirements(self, msg)
	local tagObj = self:GetNetworkVar("parent_tag") or false
	...
	if tagObj and tagObj:Exists() then
		if tagObj:GetID() ~= msg.objIDUser:GetID() and not msg.objIDUser:TeamIsOnWorldMember{i64PlayerID = tagObj}.bResult then
			if msg.isFromUI then
				...
				msg.Script_Reason = Localize("NOT_YOUR_LOOT") -- needs localization
...
function onScriptNetworkVarUpdate(self,msg) 
    for k,v in pairs(msg.tableOfVars) do
        if k == "bUsed" then
            ...
            --print("local player = " .. GAMEOBJ:GetControlledID():GetName().name)
        end
    end
end
