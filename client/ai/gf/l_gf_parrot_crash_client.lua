...
function onCollisionPhantom(self,msg)
	...
	if (player:GetID() ~= msg.objectID:GetID()) and (controlled:GetID() ~= msg.objectID:GetID()) then
		return
	end
	...
	if (self:GetVar("playerIn") == false) then
		...
		if item ~= 2641 and item ~= 2642 and item ~= 1889 then
            ...
            self:FireEventServerSide{senderID = controlled, args = "Slow"}   
        end
...
function onOffCollisionPhantom(self,msg)
	...
	if (player:GetID() ~= msg.objectID:GetID()) and (controlled:GetID() ~= msg.objectID:GetID()) then
        ...
        --if item == 2641 or item == 2642 then
            ...
            self:FireEventServerSide{senderID = controlled, args = "Unslow"}  
        --end
    end
...
function onTimerDone(self,msg)
	...
	if (msg.name == "playerEntered") then
		...
		if controlled then 
			...
			dir.x = 5--dir.x * 50
			dir.z = -60--dir.z * 50		 
			controlled:PlayFXEffect{name = "birdcavepushback", effectID = 1537, effectType = "create"}--effectID = 1378, effectType = "push-back"}
			controlled:PlayAnimation{ animationID = "knockback-recovery" }
			controlled:Knockback { vector = dir }
