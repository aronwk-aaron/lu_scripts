...
function onCollisionPhantom( self, msg )
	if msg.objectID:BelongsToFaction{factionID = 1}.bIsInFaction and not msg.objectID:GetFlag{iFlagID = playerFlag}.bFlag then
        ...
        local posOffset = -5
        local oDir = msg.objectID:GetForwardVector()
        ...
        local newPos = {x = oPos.pos.x + (oDir.niForwardVector.x * spawnDist), y = oPos.pos.y, z = oPos.pos.z + (oDir.niForwardVector.z * spawnDist)} --+ (oDir.niForwardVector.y * posOffset)
